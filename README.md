...
into HubSpot Files → Office_Screen, where the display reads them. No provider API keys. The only secret is a HubSpot Service Key
(`HUBSPOT_SERVICE_KEY`, scope `files`) in this repository's Actions secrets.
